...
class FinalizacoesGolParser:
    """Parser especialista em extrair os dados do card de Finalizações no Gol."""

    @staticmethod
    def parse(element: WebElement, comp_name: str, match: Match) -> List[FinalizacoesGolMarket]:
        resultados: List[FinalizacoesGolMarket] = []
        try:
            ...
            if "Finalizações no Gol" not in market_title:
                return resultados
            ...
            for idx in range(max(len(botoes_mais), len(botoes_menos))):
                linha_mais, odd_m = FinalizacoesGolParser._extract_linha_odd(botoes_mais[idx]) if idx < len(botoes_mais) else (0.0, 0.0)
                linha_menos, odd_n = FinalizacoesGolParser._extract_linha_odd(botoes_menos[idx]) if idx < len(botoes_menos) else (0.0, 0.0)

                if odd_m == 0.0 and odd_n == 0.0:
                    continue

                resultados.append(FinalizacoesGolMarket(
                    data=data_part,
                    hora=hora_part,
